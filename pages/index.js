...
const gold = '#d4af72'
const goldGlow = 'rgba(212,175,114,0.3)'
const goldFaint = 'rgba(212,175,114,0.15)'
    ...
    <div style={{ minHeight: '100vh', background: '#070709', fontFamily: mono, color: '#f0eef8', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link:hover { color: #d4af72 !important; transition: color 0.2s; }
        .btn-ghost:hover { border-color: #d4af72 !important; color: #d4af72 !important; transition: all 0.2s; }
      `}</style>

      {/* NAV */}
      <nav style={{ padding: '24px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(212,175,114,0.1)' }}>
        <span style={{ fontSize: 22, fontWeight: 700, letterSpacing: '0.15em', color: gold }}>✦ cnect</span>
      ...
      <div style={{ padding: '100px 48px 80px', textAlign: 'center', maxWidth: 800, margin: '0 auto', animation: 'fadeUp 0.8s both' }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <a href="#how" className="btn-ghost" style={{ background: 'transparent', color: 'rgba(240,238,248,0.5)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 50, padding: '16px 40px', fontSize: 14, letterSpacing: '0.05em', fontFamily: mono }}>
            See how it works
          </a>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', padding: '0 48px 100px', animation: 'fadeUp 0.8s 0.2s both' }}>
        <div style={{ position: 'relative' }}>
          {/* Phone */}
          <div style={{ width: 220, height: 440, background: 'linear-gradient(145deg,#1a1a14,#0f0f0a)', borderRadius: 36, border: `1px solid rgba(212,175,114,0.15)`, boxShadow: `0 40px 120px rgba(0,0,0,0.8), 0 0 0 1px ${goldFaint}`, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '36px 18px 24px', overflow: 'hidden' }}>
            <div style={{ width: 60, height: 60, borderRadius: '50%', background: `linear-gradient(135deg,${gold},#c9a55a)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: '#070709', marginBottom: 12, boxShadow: `0 0 0 3px ${goldFaint}, 0 8px 24px ${goldGlow}` }}>AR</div>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: '#f0eef8' }}>Alex Rivera</div>
            <div style={{ fontSize: 9, color: gold, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 20 }}>Creative Director</div>
            {['📸 Instagram', '🌐 Portfolio', '✉️ Email Me', '🎵 Spotify'].map((l, i) => (
              <div key={i} style={{ width: '100%', background: 'rgba(212,175,114,0.05)', border: `1px solid rgba(212,175,114,0.1)`, borderRadius: 10, padding: '10px 12px', fontSize: 11, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8, color: '#f0eef8' }}>
                {l}
              </div>
            ))}
            <div style={{ fontSize: 9, color: 'rgba(212,175,114,0.3)', marginTop: 'auto', letterSpacing: '0.08em' }}>Powered by cnect</div>
          </div>

          {/* Floating NFC card */}
          <div style={{ position: 'absolute', right: -90, bottom: 60, width: 150, height: 95, background: 'linear-gradient(135deg,#1a1a0f,#2a2510)', borderRadius: 12, border: `1px solid rgba(212,175,114,0.25)`, boxShadow: `0 20px 60px rgba(0,0,0,0.6), 0 0 30px ${goldFaint}`, padding: 14, transform: 'rotate(8deg)', animation: 'float 4s ease-in-out infinite', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: '0.15em', color: gold }}>cnect</span>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
              <span style={{ fontSize: 8, color: 'rgba(212,175,114,0.5)', letterSpacing: '0.06em' }}>Alex Rivera</span>
              <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end' }}>
                {[6,10,14].map((h,i) => <div key={i} style={{ width: 3, height: h, background: `rgba(212,175,114,0.5)`, borderRadius: 2 }} />)}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* HOW IT WORKS */}
      <div id="how" style={{ padding: '100px 48px', maxWidth: 1000, margin: '0 auto', borderTop: `1px solid rgba(212,175,114,0.08)` }}>
        <p style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: gold, marginBottom: 20 }}>How it works</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 2 }}>
          {[
            ...
            <div key={i} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)} style={{ padding: '36px 28px', background: hovered === i ? goldFaint : 'rgba(255,255,255,0.02)', border: `1px solid ${hovered === i ? `rgba(212,175,114,0.2)` : 'rgba(255,255,255,0.05)'}`, transition: 'all 0.2s', cursor: 'default', borderRadius: i === 0 ? '12px 0 0 12px' : i === 3 ? '0 12px 12px 0' : 0 }}>
              <div style={{ fontSize: 48, fontWeight: 300, color: 'rgba(212,175,114,0.15)', lineHeight: 1, marginBottom: 20 }}>{s.num}</div>
      ...
      {/* PRODUCTS */}
      <div style={{ padding: '100px 48px', maxWidth: 1000, margin: '0 auto', borderTop: `1px solid rgba(212,175,114,0.08)` }}>
        <p style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: gold, marginBottom: 20 }}>Products</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 16 }}>
          {[
            ...
            <div key={i} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid rgba(212,175,114,0.1)`, borderRadius: 16, padding: '28px 24px', position: 'relative' }}>
              {p.soon && <span style={{ position: 'absolute', top: 16, right: 16, fontSize: 9, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: gold, border: `1px solid rgba(212,175,114,0.3)`, padding: '3px 8px', borderRadius: 50 }}>Soon</span>}
              <div style={{ fontSize: 32, marginBottom: 16 }}>{p.icon}</div>
              <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>{p.name}</h3>
              <p style={{ fontSize: 13, color: 'rgba(240,238,248,0.5)', lineHeight: 1.7 }}>{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* PRICING */}
      <div id="pricing" style={{ padding: '100px 48px', maxWidth: 800, margin: '0 auto', borderTop: `1px solid rgba(212,175,114,0.08)`, textAlign: 'center' }}>
        <p style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: gold, marginBottom: 20 }}>Pricing</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 20, maxWidth: 620, margin: '0 auto' }}>
          {[
            ...
            <div key={i} style={{ background: p.featured ? goldFaint : 'rgba(255,255,255,0.02)', border: `1px solid ${p.featured ? `rgba(212,175,114,0.35)` : 'rgba(255,255,255,0.07)'}`, borderRadius: 24, padding: '40px 32px', textAlign: 'left', position: 'relative' }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: gold, marginBottom: 16 }}>{p.tier}</div>
      ...
      {/* CTA */}
      <div style={{ padding: '100px 48px', textAlign: 'center', borderTop: `1px solid rgba(212,175,114,0.08)` }}>
        <div style={{ fontSize: 48, marginBottom: 24, animation: 'float 3s ease-in-out infinite', color: gold }}>✦</div>
        ...
        <a href="mailto:hello@cnect.me" style={{ display: 'inline-block', background: `linear-gradient(135deg,${gold},#c9a55a)`, color: '#070709', borderRadius: 50, padding: '18px 48px', fontSize: 15, fontWeight: 700, letterSpacing: '0.05em', boxShadow: `0 0 40px ${goldGlow}`, fontFamily: mono }}>
          Get Your Cnect Card 🖤
        </a>
      </div>

      {/* FOOTER */}
      <div style={{ padding: '32px 48px', borderTop: `1px solid rgba(212,175,114,0.08)`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: gold, letterSpacing: '0.15em' }}>✦ cnect</span>
